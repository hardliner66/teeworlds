...
void CGameContext::StartVote(const char *pDesc, const char *pCommand, const char *pReason, CPlayer *pPlayer)
{
	...
	//if (g_Config.m_SvfCatchMinMap == 1 && strcomp(buf, "change_map") == 0)
	if (g_Config.m_SvfCatchMinMap == 1 && str_find_nocase(pCommand, "change_map") == pCommand)
	{
		time_t now;
		time(&now);
		...
		if (diffplayer / 60 < g_Config.m_SvfCatchMinMapPlayerTime)
		{
			doVote = false;
			dp = true;
		}
		...
		{
			if (diff / 60 < g_Config.m_SvfCatchMinMapTime)
			{
				doVote = false;
				dp = false;
			}
	...
	else
	{
		...
		if (dp)
		{
			seconds_remaining = g_Config.m_SvfCatchMinMapPlayerTime * 60 - diffplayer;
			str_format(aBuf, sizeof(aBuf), "Vote failed. Players must be %d minutes in game before voting. %d minutes and %d seconds remaining.", g_Config.m_SvfCatchMinMapPlayerTime, seconds_remaining / 60, seconds_remaining % 60);
		}
		else
		{
			seconds_remaining = g_Config.m_SvfCatchMinMapTime * 60 - diff;
			str_format(aBuf, sizeof(aBuf), "Vote failed. Map votes only after %d minutes. %d minutes and %d seconds remaining.", g_Config.m_SvfCatchMinMapTime, seconds_remaining / 60, seconds_remaining % 60);
		}
	...
	if (m_VoteCloseTime)
	{
		...
		else
		{
			...
			if (m_VoteUpdate)
			{
				...
				for (int i = 0; i < MAX_CLIENTS; i++)
				{
					/* fCatch - Allow voting from players in spectators (needed or the last 2 players ingame can kick the whole server),
...
void CGameContext::OnClientEnter(int ClientID)
{
	...
	{
		m_apPlayers[ClientID]->m_CaughtBy = CPlayer::FCATCH_NOT_CAUGHT;
	...
	/* fCatch begin */
	SendChatTarget(ClientID, "Welcome to fCatch!");
	...
	if (Server()->ClientIngame(ClientID))
	{
		if (MsgID == NETMSGTYPE_CL_SAY)
		{
			...
			else if (!str_comp("/help", pMsg->m_pMessage))
			{
				...
				SendChatTarget(ClientID, " ");
				SendChatTarget(ClientID, "In advanced mode, you have to freeze your enemies first by shooting them,");
				SendChatTarget(ClientID, "then throw them in the air with the hook");
				SendChatTarget(ClientID, "and then kill them by shooting again!");
		...
		else if (MsgID == NETMSGTYPE_CL_CALLVOTE)
		{
			...
			// fCatch - Only People who are explicit in Spectators can't vote!
			if (pPlayer->m_SpecExplicit == 1) //fCatch
			{
			...
			else if (str_comp_nocase(pMsg->m_Type, "kick") == 0)
			{
				...
				if (g_Config.m_SvVoteKickMin)
				{
					int PlayerNum = 0;
					for (int i = 0; i < MAX_CLIENTS; ++i)
						if (m_apPlayers[i] && m_apPlayers[i]->m_SpecExplicit != 1) // fCatch - Count all Players who are not explicit in spectator
							++PlayerNum;
...
void CGameContext::OnInit(/*class IKernel *pKernel*/)
{
	...
	else if(str_comp_nocase(g_Config.m_SvGametype, "fcatch") == 0)
		m_pController = new CGameController_fCatch(this);
	else
