...
void *CDataFileReader::GetDataImpl(int Index, int Swap)
{
	...
	if(!m_pDataFile->m_ppDataPtrs[Index])
	{
		...
		if(m_pDataFile->m_Header.m_Version == 4)
		{
			...
			dbg_msg("datafile", "loading data index=%d size=%d uncompressed=%ld", Index, DataSize, UncompressedSize);
...
int CDataFileWriter::Finish()
{
	...
	{
		if(DEBUG)
			dbg_msg("datafile", "item=%d size=%d (%ld)", i, m_pItems[i].m_Size, m_pItems[i].m_Size+sizeof(CDatafileItem));
	...
	// construct Header
	{
		...
		// write Header
		if(DEBUG)
			dbg_msg("datafile", "HeaderSize=%ld", sizeof(Header));
#if defined(CONF_ARCH_ENDIAN_BIG)
		swap_endian(&Header, sizeof(int), sizeof(Header)/sizeof(int));
#endif
